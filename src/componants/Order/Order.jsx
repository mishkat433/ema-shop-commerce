            ...
            <div className='flex flex-col-reverse lg:flex-row w-11/12 gap-6 mx-auto mt-10' >
                ...
                <div className="cart-container  w-full lg:w-2/5 rounded-xl pb-10 lg:pb-0 lg:h-[100vh] lg:sticky top-0 ">
                    <ShoppingCart cart={carts} setCart={setCart}>
                        {carts.length === 0 ? <h3 className='mt-5 text-center text-red-500'>you have not select any product</h3> :
                            <Link to="/shipping"><div className='flex justify-between px-5 mt-5'>
                                <button className="btn   bg-cyan-700 border-2  hover:bg-cyan-500 border-none hover:text-white  w-full px-5  duration-200" onClick={removeHandle}>
                                    Checkout </button>
                            </div></Link>}
                    </ShoppingCart>
                </div>
